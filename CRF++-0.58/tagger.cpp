...
bool TaggerImpl::shrink() {
  ...
  // add booker
  std::vector<TagInfo>(tags_).swap(tags_);
  ...
  if (nbest_ >= 1) {
    ...
    if (vlevel_ >= 1) {
      ...
      for(size_t idx = 0; idx < tags_.size(); ++idx){
          if(idx != 0) os_ << ";";
          os_ << tags_[idx].offset << "," << tags_[idx].length << "," << tags_[idx].prob;
      }
      os_ << '\n';
    }
    PRINT;
...
//add booker
void TaggerImpl::createNe(){
    for (size_t i = 0; i < x_.size();) {
        std::string tag = yname(y(i));
        if(tag[0] == 'B' || tag[0] == 'b'){
            size_t idm = i + 1;
            while(idm < x_.size()){
                tag = yname(y(idm));
                if(tag[0] == 'o' || tag[0] == 'O' || tag[0] == 'B' || tag[0] == 'b'){
                    break;
                }else{
                  idm += 1;
                }
            }
            TagInfo tag_info(i, idm - i);
            for(size_t idy = i; idy < idm; ++idy){
                tag_info.tags.push_back(y(idy));
            }
            tags_.push_back(tag_info);
            i = idm;
        }else if (tag[0] == 'S' || tag[0] == 's'){
            TagInfo tag_info(i, 1);
            tag_info.tags.push_back(y(i));
            tags_.push_back(tag_info);
            i += 1;
        }else{
            i += 1;
...
// add booker
void TaggerImpl::neProb(){
    // score path
    for(size_t idx = 0; idx < tags_.size(); ++idx){
        size_t begin = tags_[idx].offset;
        size_t len = tags_[idx].length;
        size_t end = begin + tags_[idx].length - 1;
        double Z0 = 0.0;
        const std::vector<size_t>& s_answer = tags_[idx].tags;
        // forward alphat + begin + score path + end
        Z0 += node_[begin][s_answer[0]]->alpha;
        for(size_t idy = 1; idy < len; ++idy){
            size_t nx = begin+idy;
            Z0 += node_[nx][s_answer[idy]]->cost; // UNIGRAM cost
            const std::vector<Path *> &lpath = node_[nx][s_answer[idy]]->lpath;
            for (const_Path_iterator it = lpath.begin(); it != lpath.end(); ++it) {
                if ((*it)->lnode->y == s_answer[idy - 1]) {
                    Z0 += (*it)->cost;  // BIGRAM COST
                    break;
                }
            }// end for
        }
        // backword
        Z0 += (node_[end][s_answer[len-1]]->beta - node_[end][s_answer[len-1]]->cost);
        tags_[idx].prob =  std::exp(Z0 - Z_);
    }
}
